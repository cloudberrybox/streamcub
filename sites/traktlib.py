...
# Trakt
apikey = '8dcf1bc00c9d09e8bd6e05470f5413f2'
...
def createListTrakt(name, type):
    args = {}
    args['name'] = type + ' ' + name
    args['description'] = ''
    args['privacy'] = 'public'
    args['show_numbers'] = 'true'
    args['allow_shouts'] = 'true'

    data = traktJsonRequest('POST', '/lists/add/%%API_KEY%%',args)
    if data == None:
        print("Error in request from 'createListTrakt()'")
    return data
...
def getRecommendedShowsFromTrakt(genre):
    ...
    if data == None:
	    print("Error in request from 'getRecommendedShowsFromTrakt()'")
    return data
...
def getListsFromTrakt():
    data = traktJsonRequest('POST', '/user/lists.json/%%API_KEY%%/%%USERNAME%%')
    if data == None:
        print("Error in request from 'getListsFromTrakt()'")
    return data
...
def getShowInfobySearch(title):
    ...
    if data == None:
        print("Error in request from 'getShowInfobySearch()'")
    return data
...
def getShow(tvdb_id):
	fullshow = traktJsonRequest('POST', '/show/summary.json/%%API_KEY%%/{0}'.format(tvdb_id))
	return fullshow
